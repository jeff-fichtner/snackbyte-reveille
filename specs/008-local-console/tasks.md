...
- **T050 PASS — in full, including the manual half.** Automated: the console was killed mid-watch and the launch was **unaffected** (Palworld reached `running` on its own). Manual, performed by the operator at a real terminal on 2026-08-10: a live Ctrl-C during `reveille start satisfactory` printed *"Stopped watching. The launch was already issued and is unaffected."* and Satisfactory reached `running` regardless. **Both halves of FR-019 confirmed; nothing about this feature remains unexercised.**
  - Papercut noted, not a defect: because the shim is a `.cmd`, Ctrl-C also raises `cmd.exe`'s *"Terminate batch job (Y/N)?"* after the sentence. Standard for any batch wrapper (npm's behaves the same) and it changes no behaviour, but it makes the interrupt read messier than the spec implies. Deferred rather than improvised.
