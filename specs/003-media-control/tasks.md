...
- [ ] T005 Select the adapter by `TARGET` in `agent/src/config.ts` — read `TARGET` (renamed from `GAME`): `palworld`/`satisfactory` → their game adapters, `vlc` → the media adapter; require the chosen target's config block fail-loud, naming each. The game agents behave identically (only the variable name changed); the running `agent/.env.palworld`/`.env.satisfactory` migrate `GAME=` → `TARGET=` at deploy (like 002's `GAME` migration). `agent/src/index.ts` builds the selected adapter at boot
